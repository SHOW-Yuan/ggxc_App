			...
			<block v-for="(item,index) in listDatas" :key="index">
				<view class="item-box">
					<view class="item-box-one">
						...
						<view>
							<text>{{item.FuelingCapacity}}/升</text>
							<text>用量</text>
						</view>
						<view>
							<text>{{item.OilPrice}}/升</text>
							<text>单价</text>
						</view>
						<view>
							<text>{{item.OilMoney}}￥</text>
							<text>燃料金额</text>
						</view>
					</view>
					<view class="item-box-two">
						<text>{{item.ReportingTime}}</text>
						<text>{{item.coach}}</text>
						<text @click="auditClick(item)" v-if="item.auditState == 0">未审核</text>
...
<script>
	...
	export default {
		onLoad(opction) {
			...
			this.formData.datas[0].cphm = opction.cphm;
			this.formData.datas[0].date = opction.date;
			this.loadData();
		},
		...
		methods: {
			loadData(){
				uni.request({
					url:this.$ip.url.ip1 + "/app/menmian-area/4QAAAAEBAB4/FuelReport",
					method:'post',
			...
			auditClick(item){
				this.text = item.coach + ' 给 车辆' + item.cphm  + '添加燃料' + item.OilMoney + '元'
				this.item = item
				this.show = true
			...
			bindBtn(val){
				...
				console.log(val);
				uni.request({
					url:this.$ip.url.ip1 + "/app/menmian-area/4QAAAAEBAB4/FuelReportAudit",
					method:'post',
